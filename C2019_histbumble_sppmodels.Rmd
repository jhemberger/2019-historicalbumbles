...
species.2.glms <- function(data) {
  ...
  for (i in species.list) {
    ...
    predict <- predictorEffects(glm.model,
                                axes = list(x = list(rug = TRUE)),
                                se = list(compute = TRUE))
    predict.df <- bind_rows(as.tibble(predict$pastureac) %>%
                              rename(new_x = pastureac) %>%
                              mutate(var = "pastureac"),
                            as.tibble(predict$pestac) %>%
                              rename(new_x = pestac) %>%
                              mutate(var = "pestac"),
    ...
    pestac.plot <- ggplot() + 
      ...
      #                    breaks = seq(1870, 2020, by = 20)) +
      ylab(label = "Proportional Abundance") + 
      xlab(label = "Pesticide Treated Acreage") + 
      labs(title = i)
    # ggsave(plot = ncrop.plot,
    #        filename = paste("./model_plots/ncrops/",
    #                         i,
...
spatial.bumbles.glmvars2.df <- spatial.bumbles.glmvars.df %>%
  dplyr::select(-c(n_crops, crop_evenness, prop_cropland, fertexp, covercropac)) %>%
  drop_na(cloverac, pastureac, pestac, fertac) %>%
...
## Prep data
```{r}
spatial.bumbles.range.df <- spatial.bumbles.df %>%
  ungroup() %>%
  select(species, subgenus, state, county, bin_8) %>%
  group_by(species, state) %>%
  distinct(county, .keep_all = TRUE) %>%
  group_by(species, bin_8) %>%
...
spatial.bumbles.range.df


# alternate way not filtering by min records per county (just present or)
...
# ggsave(paste("./model_plots/county_trends/", i, ".eps", sep = ""),
  #        plot,
  #        height = 3,
  #        width = 5)
...
range.glm.plot1 <- glms3.df %>%
  ...
  theme_minimal() +
  coord_flip() + 
  ggsave("./model_plots/coefs/n_couniest.eps",
       range.glm.plot1,
       width = 5,
       height = 6)
range.glm.plot1
