...
midwestag.plot <- ggplot() +
  ...
  theme_void()
midwestag.plot
ggsave("./model_plots/agovertime.eps",
       midwestag.plot,
       width = 10,
       height = 10)
```

## Plot crop richness over time
```{r}
midwestag.richness.plot.df <- midwestag.richness.df %>%
  left_join(us.county.midwest %>%
              mutate(CNTY_NAME = paste(CNTY_NAME,
                                       "County",
                                       sep = " ")),
            by = c("COUNTY" = "CNTY_NAME"))
midwestag.richness.plot.df

midwestag.richness.plot <- ggplot() +
  geom_polygon(data = midwestag.richness.plot.df,
               mapping = aes(x = long,
                             y = lat,
                             group = group,
                             fill = n_crops),
  ...
  coord_map("stereographic") + 
  labs(title = "Number of crops grown per county") + 
  facet_wrap(~ year,
             ncol = 5) +
  theme_void()
midwestag.richness.plot
