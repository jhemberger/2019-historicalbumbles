...
```{r}
histag2.df <- read_tsv("./data/AgCensus_MasterDataFrame_2020.txt")
```
...
histag2.df <- histag2.df %>%
  ...
  left_join(cropcode.df,
            by = c("crop" = "crop_abbr"))
histag2.df <- histag2.df %>%
  filter(!crop %in% c("Cat", "Hrs", "Hmn", "Shp", "Swn", "Tky", "Ckn"))
...
midwestag.countysize.df <- midwestag.bycrop.df %>%
  ...
  mutate(area_acres = area_km * 247.105,
         state_cty = paste(state, county, sep = "_")) %>%
  distinct(state_cty, .keep_all = TRUE) %>%
  write_csv("./data/midwest_countysize.csv")
...
range(midwestag.bycrop.df$prop_county)
...
```{r}
midwestag.othervars2.df <- midwestag.other.df %>%
  left_join(midwestag.countysize.df %>%
              mutate(state = abbr2state(state)) %>%
              mutate(state = tolower(state),
                     county = tolower(county)), ###CHANGE THIS TO LEFT JOIN 
            by = c("state" = "state",
                   "county" = "county")) %>%
  mutate(prop_pasture = pastureac / area_acres,
         prop_pest = pestac / area_acres,
         prop_cc = covercropac / area_acres) %>% 
  filter(!county_name %in% c("Wisconsin", "Iowa", "Illinois", "Indiana", "Michigan", "Minnesota")) %>%
  write_csv("./data/midwest_agstats_other.csv", append = FALSE)

unique(midwestag.othervars2.df$prop_pasture)
```
...
```{r}
proppast.plot <- midwestag.map.df %>%
ggplot() + 
  # geom_point(mapping = aes(x = year, 
  #                          y = n_crops,
  #                          color = STATE)) + 
  ...
  scale_color_manual(values = paste(carto_pal(n = 6, "Fall"), sep = ",")) +
  theme_minimal()
proppast.plot
ggsave("./model_plots/trends/proppast_trend.eps",
       plot = proppast.plot,
       height = 3,
       width = 4)
```
...
```{r}
proppest.plot <- midwestag.map.df %>%
ggplot() + 
  # geom_point(mapping = aes(x = year, 
  #                          y = n_crops,
  #                          color = STATE)) + 
  ...
  theme_minimal()
proppest.plot
ggsave("./model_plots/trends/proppest_trend.eps",
       plot = proppest.plot,
       height = 3,
       width = 4)
```
...
```{r}
no_classes = 8
pcc_quantiles <- midwestag.othervars2.df %>%
  filter(!county_name %in% c("Wisconsin", "Iowa", "Illinois", "Indiana", "Michigan", "Minnesota"))
pcc_quantiles <- stats::quantile(pcc_quantiles$prop_cc, 
                                 na.rm = TRUE, 
                                 type = 6,
                                 probs = seq(0, 1, length.out = no_classes + 1))
pcc_quantiles
pcc_labels <- c("0-0.0004", 
                "0.0005-0.0006", 
                "0.0007-0.004", 
                "0.005-0.007", 
                "0.008-0.012", 
...
midwestag.othervars2.df$pcc_brks <- cut(midwestag.othervars2.df$prop_cc, 
                                          breaks = pcc_quantiles, 
                                          include.lowest = TRUE,
                                          labels = pcc_labels)
pcc_breaks_scale <- levels(midwestag.othervars2.df$pcc_brks)

propcc.plot <- midwestag.othervars2.df %>%
  ...
  geom_polygon(data = us.county.midwest,
               ...
               size = 0.25) +
  scale_fill_manual(values = rev(viridis::inferno(8, direction = -1)),
                    breaks = rev(pcc_breaks_scale),
                    name = "Proportion county in cover crop",
                    drop = FALSE,
                    labels = rev(pcc_breaks_scale)) +
  # scale_fill_gradient(low = "#FFFFFF",
  #                     high = "#F9A242",
  #                     na.value = "#FFFFFF",
  ...
  theme_void()
propcc.plot
ggsave("./model_plots/prop_cc.eps",
       propcc.plot,
       width = 10)
...
ggplot() + 
  ...
  #                    colour = "red", size = 0.5) + 
  xlab(label = "Year") +
  ylab(label = "Proportion county in cover crop") + 
  scale_x_continuous(limits = c(1982, 2012),
                     breaks = seq(1982, 2012, by = 10)) + 
  scale_color_viridis_d(option = "inferno") + 
  #scale_color_gradientn(colors = viridis::inferno(6)) + 
  theme_histbumbles() +
  ggsave("./model_plots/pcc_trend.eps",
         height = 2,
         width = 4)
...
midwestag.othervars2.df %>%
  filter(year > 1974) %>%
  group_by(year) %>%
  summarise(mean = mean(prop_cc, na.rm =TRUE),
            se = sd(prop_cc, na.rm = TRUE) / sqrt(n()))
```
