...
spatial.bumbles.df <- spatial.bumbles.df %>%
  mutate(bin_ag = case_when(
    ...
    between(year, 2006, 2014) ~ 2012,
    between(year, 2015, 2019) ~ 2017),
...
spatial.bumbles.glmvars.2yr.df <- spatial.bumbles.2yr.ra.df %>%
  ...
  left_join(midwestag.model.df,
            by = c("bin_ag" = "year",
                   "state" = "state",
                   "county" = "county")) %>%
  filter(bin_ag_abundance > 2) # tuner for minimum number of records per county
...
spatial.bumbles.ra.red.df <- spatial.bumbles.red.df %>%
  ...
  summarise(species_abun = n()) %>%
  ungroup() %>%
  left_join(spatial.bumbles.red.df %>%
              group_by(state, county, bin_ag) %>%
              summarise(bin_ag_abundance = n()),
            by = c("bin_ag" = "bin_ag",
                   "state" = "state",
...
```{r}
spatial.bumbles.glmvars.df
spatial.corr.test.df <- spatial.bumbles.glmvars2.df %>%
...
## Spatial AutoC Function
## Fit SARMA Models
```{r}
spatial.corr.test <- function(spdf) {
  analysis.spdf <- spdf %>%
  ...
  sarma.results.list <- list()
  sarma.models.list <- list()
  for (i in spdf.list) {
    listnumber <- listnumber + 1
    ols.model <- lm(rel_abun ~ n_crops + prop_cropland + bin_ag,
                    data = i@data,
    ...
    # print(moran.test$p.value)
    # print(ols.df)
    legrange.tests <- lm.LMtests(ols.model,
                                 weights,
                                 test = "all",
                                 zero.policy = TRUE)
    # print(legrange.tests)
    ...
    fit.sma <- errorsarlm(rel_abun ~ 
                          ...
                          listw = weights,
                          zero.policy = TRUE)
    sarma.model.df <- tibble(term = c("(Intercept)",
                                      "n_crops",
                                      "prop_cropland",
                                      "bin_ag"),
      ...
      mutate(model.type = "SARMA",
             species = i@data$species[1],
             obsv = nrow(i@data)) %>%
      mutate(moran_pvalue = moran.test$p.value) %>%
      dplyr::select(species, model.type, obsv, moran_pvalue, everything())
    sarma.results.list[[listnumber]] <- sarma.model.df
    sarma.models.list[[listnumber]] <- fit.sma
  }
  ols.results.df <- bind_rows(modelresults.list)
  sarma.results.df <- bind_rows(sarma.results.list) %>%
    filter(moran_pvalue < 0.05)
  assign("ols.spatialauto.df",
         ols.results.df,
         envir = .GlobalEnv)
  assign("sarma.spatialauto.df",
         sarma.results.df,
         envir = .GlobalEnv)
  assign("sarma.models.list",
         sarma.models.list,
         envir = .GlobalEnv)
}
```

## Run spatial.corr.test function
```{r}
spatial.corr.test(spatial.corr.test.spdf)
sarma.models.list[[1]] <- NULL
names(sarma.models.list) <- species
```

## Which specices are spatially autocorrelated?
```{r}
spatialautocor.df %>%
  group_by(species) %>%
  filter(moran_pvalue < 0.05) %>%
...
```{r}
sarma.models.list[[2]]
summ(sarma.models.list[[2]],
     scale = TRUE,
     confint = TRUE)

sarma.spatialauto.df
```
...
sarma.spatialauto.df %>%
  ...
  #                                 2.5,
  #                                 by = 2)) +
  facet_grid(vars(term), scales = "free") +
  theme_histbumbles() +
  coord_flip()
```

## Predict relative abundance
```{r}
pred.relabun.spatial <- function(model.list) {
  pred.list <- list()
  years.list <- list()
  ...
  for (i in 1:length(model.list)) {
    for(j in years) {
      newdata.df <- midwestag.model.df %>%
        filter(year == j) %>%
        mutate(bin_ag = year)
      predict <- predict.sarlm(object = model.list[[i]],
                         newdata = newdata.df,
                         na.action = na.pass)
  ...
  assign("relabun.bycty.spatial.df",
         relabun_bycty.df,
         envir = .GlobalEnv)
}
pred.relabun(sarma.models.list)
...
coef.propag.plot <- glms.df %>%
  ...
  coord_flip()
coef.propag.plot
ggsave("./model_plots/coefs/prop_cropland_2yr.eps",
       coef.propag.plot,
       width = 4,
       height = 4)
```
...
coef.ncrop.plot <- glms.df %>%
  ...
  coord_flip()
coef.ncrop.plot
ggsave("./model_plots/coefs/n_crops_2yr.eps",
       coef.ncrop.plot,
       width = 4,
       height = 4)
...
coef.year.plot <- glms.df %>%
  ...
  coord_flip()
coef.year.plot
ggsave("./model_plots/coefs/year_2yr.eps",
       coef.year.plot,
       width = 4,
       height = 4)
```
...
## Prep data - THIS IS WRONG!
```{r}
spatial.bumbles.diverity.df <- spatial.bumbles.df %>% # THIS IS WRONG! 
  group_by(state, county, bin_ag) %>%
  distinct(species, .keep_all = TRUE) %>%
...
## Prep data
```{r}
spatial.bumbles.diversity.df <- spatial.bumbles.glmvars2.df %>%
  group_by(bin_ag, county, state) %>%
  distinct(species, .keep_all = TRUE) %>%
  summarise(n_species = n()) %>%
  left_join(spatial.bumbles.glmvars2.df %>%
              dplyr::select(state, county, bin_ag, prop_cropland, n_crops, prop_pasture, prop_pest) %>%
              distinct(paste(bin_ag, county, state), .keep_all = TRUE),
            by = c("state" = "state",
                   "county" = "county",
                   "bin_ag" = "bin_ag")) %>%
  left_join(spatial.bumbles.df %>%
...
diversity.glms <- function(data) {
  ...
  predict.df <- bind_rows(as.tibble(predict$n_crops) %>%
                          ...
                          as.tibble(predict$prop_pasture) %>%
  ...
  proppast.plot <- ggplot() + 
    ...
    labs(title = i)
  ggsave(plot = proppast.plot,
         filename = "./model_plots/diversity/pasture/diversity.eps",
         height = 3,
         width = 5)

  proppest.plot <- ggplot() + 
    geom_point(data = model.df ,
    ...
    labs(title = i)
  ggsave(plot = proppest.plot,
         filename = "./model_plots/diversity/pest/diversity.eps",
         height = 3,
         width = 5)
...
## Run GLM Function
```{r}
diversity.glms(spatial.bumbles.diversity.df)
```

## Test for spatial autocorrelation
```{r}
spatial.corr.test.df <- spatial.bumbles.diversity.df %>%
  mutate(state_full = abbr2state(state)) %>%
  left_join(countycode.df,
            by = c("state_full" = "state",
                   "county" = "county_name"))
spatial.corr.test.spdf <- sp::merge(midwest.spdf,
                                  ...
                                  by = "GID_2",
                                  duplicateGeoms = TRUE)
spatial.corr.test.spdf <- spatial.corr.test.spdf %>%
  filter(!is.na(n_species))

ols.model <- lm(n_species ~ n_crops + prop_cropland + bin_ag + prop_pasture + prop_pest,
                data = spatial.corr.test.spdf@data,
                weights = n_records,
                na.action = na.exclude)
ols.df <- tidy(ols.model) %>%
  mutate(model.type = "ols",
         obsv = nrow(spatial.corr.test.spdf@data)) %>%
  dplyr::select(model.type, obsv, everything())
# residuals(ols.model)

weights.queen.list <- poly2nb(spatial.corr.test.spdf,
                              queen = TRUE)
# print(length(weights.queen.list))
# print(length(ols.model$residuals))
# acf(residuals(ols.model))
...
fit.sma <- spautolm(n_species ~ scale(n_crops) + 
                      scale(prop_cropland) +
                      scale(prop_pasture) + 
                      scale(prop_pest) +
                      scale(bin_ag),
                    data = spatial.corr.test.spdf,
                    weights = n_records,
                    listw = weights,
                    family = "SMA")
sarma.model.df <- tibble(term = c("(Intercept)",
  ...
  mutate(model.type = "SARMA",
         obsv = nrow(spatial.corr.test.spdf@data)) %>%
  mutate(moran_pvalue = moran.test$p.value) %>%
  dplyr::select(model.type, obsv, moran_pvalue, everything())
```
...
#### Generate range of species
```{r}
analysis.bumbles.df %>%
  filter(species == "Bombus terricola") %>%
  dplyr::select(state, county) %>%
  mutate(state = abbr2state(state)) %>%
  left_join(ungroup(us.county.midwest),
...
```{r}
test <- relabun_bycty.df %>%
  filter(year %in% c(1850, 1900, 1950, 1974, 2012)) %>%
  group_by(species, year) %>%
  summarise(mean = mean(rel_abun),
            se = sd(rel_abun) / sqrt(n())) %>%
  filter(species %in% c("Bombus bimaculatus",
                        "Bombus griseocollis",
                        "Bombus impatiens", 
                        "Bombus affinis",
                        "Bombus pensylvanicus",
...
### Quantile labels
```{r}
stats::quantile(x = relabun_bycty.df$rel_abun,
                                    na.rm = TRUE,
                                    type = 1,
...
plot.predicted <- function() {
  plot.species <- c("Bombus affinis",
                        ...
                        "Bombus terricola",
                        "Bombus vagans")
  prob_quantiles <- stats::quantile(x = relabun_bycty.df$rel_abun,
                                    na.rm = TRUE,
                                    type = 1,
                                    probs = seq(0, 1, length.out = 9))
  prob_labels <- c("0.000-0.017",
                   "0.018-0.043",
                   "0.045-0.068",
                   "0.069-0.102",
                   "0.103-0.151",
                   ...
                   "0.225-0.365",
                   "0.366-0.999")
  relabun_bycty.df$prob_brks <- cut(relabun_bycty.df$rel_abun,
                                    breaks = prob_quantiles,
                                    include.lowest = TRUE,
                                    labels = prob_labels)
  prob_brks_scale <- levels(relabun_bycty.df$prob_brks)
  bumblepred.df <- relabun_bycty.df
  ...
  for(i in plot.species) {
    ...
    #              size = 0.75) +
      ...
      scale_color_manual(values = rev(viridis::inferno(8, direction = 1)),
                        breaks = rev(prob_brks_scale),
                        name = "Probability of occupancy",
                        drop = FALSE,
                        labels = rev(prob_brks_scale)) +
  ...
  for (i in species.list) {
    ...
    predict.df <- bind_rows(as.tibble(predict$prop_pasture) %>%
                            ...
                            as.tibble(predict$prop_pest) %>%
                              rename(new_x = prop_pest) %>%
                              mutate(var = "prop_pest"),
                            # as.tibble(predict$prop_cc) %>%
                            #   rename(new_x = prop_cc) %>%
                            #   mutate(var = "prop_cc"),
                            as.tibble(predict$bin_ag) %>%
                              rename(new_x = bin_ag) %>%
                              mutate(var = "bin_ag"))
...
spatial.bumbles.glmvars2.df <- spatial.bumbles.glmvars.df %>%
  ungroup() %>%
  dplyr::select(species, subgenus, state, county, bin_ag, species_abun, bin_ag_abundance,
         rel_abun, n_crops, prop_cropland) %>%
  left_join(midwestag.othervars2.df %>%
              filter(!is.na(year)) %>%
              mutate(bin_ag = year) %>%
              dplyr::select(bin_ag, state_name, county_name, avgfarm, cloverac, pestac, pastureac, prop_pasture, prop_pest, prop_cc),
            by = c("state" = "state_name",
                   "county" = "county_name",
                   "bin_ag" = "bin_ag")) %>%
...
unique(midwestag.othervars2.df$state_name)
...
### Print VIFs
```{r}
for (i in 1:length(glm.output2.list)) {
  print(names(glm.output2.list[i]))
  print(car::vif(glm.output2.list[[i]]))
}
```
...
ggsave("./model_plots/coefs/pestac.eps",
       ...
       width = 4,
       height = 4)
range(spatial.bumbles.glmvars2.df$prop_cc, na.rm = TRUE)
```
#### cover crop
```{r}
coef.propcc.plot <- glms2.df %>%
  # filter(n_records > 60) %>%
  filter(term == "prop_cc") %>%
  arrange(desc(estimate)) %>%
  mutate(row_order = factor(species,
...
county.time.trend.plot <- spatial.bumbles.range.df %>%
  group_by(species) %>%
  mutate(order = factor(species, levels = c("Bombus fervidus",
                                            "Bombus terricola",
                                            "Bombus pensylvanicus",
                                            "Bombus affinis",
                                            "Bombus borealis",
...
```{r}
spatial.bumbles.rangera.df <- bumbles.df %>%
  ungroup() %>%
  dplyr::select(species, subgenus, state, county, bin_15) %>%
  group_by(species, state) %>%
  distinct(county, .keep_all = TRUE) %>%
...
left_join(bumbles.df %>%
              group_by(species, bin_15) %>%
              summarise(bin_15_abundance = n()),
            by = c("bin_15" = "bin_15",
                   "species" = "species"))
species.range.plot <- spatial.bumbles.rangera.df %>%
  ggplot() +
  geom_point(mapping = aes(x = year,
                           y = county_ra),
             shape = 21,
  ...
  theme_histbumbles()
species.range.plot
ggsave("./model_plots/county_change.eps",
       species.range.plot,
       width = 7,
       height = 4.5)
```
...
range.lms <- function(data) {
  ...
  assign("range.lms.df",
         dfs,
         envir = .GlobalEnv)
    assign("range.lms.list",
         lm.list,
         envir = .GlobalEnv)
}

range.lms(spatial.bumbles.rangera.df)
```
...
range.glms <- function(data) {
  ...
  for (i in species) {
    model.df <- data %>%
      filter(species == i)
    glm <- glm(county_ra ~ year,
               data = model.df,
               weights = model.df$bin_15_abundance,
               family = "poisson")
    # plot(glm)
    tidy.df <- tidy(glm) %>%
      mutate(species = i) %>%
...
range.glms(spatial.bumbles.rangera.df)
```
...
### Plot coefs (lms)
```{r}
scale_coefs4 <- function(model.list) {
  df.list <- list()
  iter <- 0
  for (i in model.list) {
    iter <- iter + 1
    summ.lm <- summ(i,
                     scale = TRUE,
                     confint = TRUE)
    lm.df <- as.data.frame(summ.lm[["coeftable"]]) %>%
      mutate(term = c("(Intercept)",
                      "year")) %>%
      dplyr::select("term" = "term",
                    "estimate" = "Est.",
                    ...
                    "p.value" = "p") %>%
      mutate(species = names(model.list[iter]),
             sample.size = length(summ.lm$model$y)) %>%
      dplyr::select(species, sample.size, everything())
    print(lm.df)
    df.list[[iter]] <- lm.df
  }
  lms.df <- bind_rows(df.list)
  assign("rangelm.df",
         lms.df,
         envir = .GlobalEnv)
}

scale_coefs4(range.lms.list)

range.lm.plot <- rangelm.df %>%
  filter(term == "year") %>%
  arrange(desc(estimate)) %>%
  mutate(row_order = factor(species,
                            levels = species)) %>%
...
ggplot(data = spatial.bumbles.range.df) + 
  geom_point(mapping = aes(x = bin_15,
                           y = n_counties,
                           fill = species),
             shape = 21,
             color = "black") +
...
# Test corrected data from Mike (county area)
```{r}
test.ag.df <- read_tsv("../../../Desktop/AgCensus_MasterDataFrame.txt")

str(test.ag.df)
unique(test.ag.df$STATE
       )

test.ag.df %>%
  dplyr::select(COUNTY, STATE, AREA_KM) %>%
  filter(STATE == "IL" & COUNTY == "Adams County")
```
