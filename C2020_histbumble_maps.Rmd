...
midwestag.map.df <- read_csv("./data/midwest_agstats.csv") %>%
...
```{r}
no_classes = 8
pcrop_quantiles <- stats::quantile(midwestag.map.df$prop_cropland, 
                            na.rm = TRUE, 
                            type = 6,
...
midwestag.map.df$pcrop_brks <- cut(midwestag.map.df$prop_cropland, 
                                    breaks = pcrop_quantiles, 
                                    include.lowest = TRUE,
                                    labels = pcrop_labels)
pcrop_brks_scale <- levels(midwestag.map.df$pcrop_brks)

propcrop.plot <- midwestag.map.df %>%
  ...
  geom_polygon(data = state_df <- map_data("state") %>%
               ...
               size = 1.25,
               fill = NA) +
  scale_fill_manual(values = rev(cal_palette(name = "creek", n = 8, type = "continuous"))) +
...
```{r}
no_classes = 8
ncrop_quantiles <- stats::quantile(midwestag.map.df$n_crops, 
                            na.rm = TRUE, 
                            type = 1,
...
midwestag.map.df$ncrop_brks <- cut(midwestag.map.df$n_crops, 
                                    breaks = c(0, 4, 8, 10, 11, 12, 14, 16), 
                                    include.lowest = TRUE,
                                    labels = ncrop_labels)
ncrop_brks_scale <- levels(midwestag.map.df$ncrop_brks)


even_quantiles <- stats::quantile(midwestag.map.df$crop_evenness, 
                            na.rm = TRUE, 
                            type = 1,
                            probs = seq(0, 1, length.out = no_classes))
even_quantiles
...
midwestag.map.df$even_brks <- cut(midwestag.map.df$crop_evenness, 
                                    breaks = even_quantiles, 
                                    include.lowest = TRUE,
                                    labels = even_labels)
even_brks_scale <- levels(midwestag.map.df$even_brks)

ncrop.plot <- midwestag.map.df %>%
  filter(year %in% c(1880, 1900, 1950, 1974, 2017)) %>%
  ggplot() + 
  geom_polygon(mapping = aes(x = long,
                             y = lat,
  ...
  geom_polygon(data = state_df <- map_data("state") %>%
               ...
               size = 1.25,
               fill = NA) +
  scale_fill_manual(values = cal_palette(name = "creek", n = 8, type = "continuous")) +
  # scale_fill_manual(values = rev(viridis::inferno(8, direction = -1)),
